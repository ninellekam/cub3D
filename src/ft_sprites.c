...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/10 17:23:33 by ninakamkia        #+#    #+#             */
/*   Updated: 2021/02/10 18:30:34 by ninakamkia       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include "game.h"
 ...
 void	ft_final_sprite(t_game *g, t_sprites *s)
 {
 	...
 	while (g->sprites.hi < s->end_y)
 	{
 		...
 		color = ((int*)g->sprites.tex.img.addr)
 ...
 void	ft_draw_sprite(t_game *g, t_sprites *s, double *buf)
 {
 	...
 	while (g->sprites.ok < s->end_x)
 	{
 		...
 		g->sprites.texX = (int)256 * (g->sprites.ok - (-s->s_w / 2 + s->screenX));
 		g->sprites.texX = g->sprites.texX * g->sprites.tex.X / s->s_w / 256;
 		if (s->transformY > 0 && g->sprites.ok > 0 && g->sprites.ok < g->map.R[0]
